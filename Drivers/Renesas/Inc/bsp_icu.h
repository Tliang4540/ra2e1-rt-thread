...
#define bsp_irq_status_clear(irq)   (R_ICU->IELSR_b[irq].IR = 0U)

/* Enable the IRQ in the NVIC (Without clearing the pending bit). */
#define bsp_irq_enable(irq)         (NVIC->ISER[(((uint32_t) irq) >> 5UL)] = (uint32_t) (1UL << (irq & 0x1FUL)))

/* Clears pending interrupts in both ICU and NVIC */
#define bsp_irq_clear_pending(irq)  (NVIC->ICPR[(((uint32_t) irq) >> 5UL)] = (uint32_t) (1UL << (irq & 0x1FUL)))

/* Disables interrupts in the NVIC. */
#define bsp_irq_disable(irq)        {NVIC->ICER[(((uint32_t) irq) >> 5UL)] = (uint32_t) (1UL << (irq & 0x1FUL));__DSB();__ISB();}

typedef enum
{
    IRQ_MODE_FALLING = 0x00,
    IRQ_MODE_RISING  = 0x01,
    IRQ_MODE_FALLING_RISING = 0x02,
    IRQ_MODE_LOW_LEVEL = 0x03,
    IRQ_MODE_FILTER_ENABLE  = 0x80,
    IRQ_MODE_FILTER_PCLKB8 = 0x90,
    IRQ_MODE_FILTER_PCLKB32 = 0xA0,
    IRQ_MODE_FILTER_PCLKB64 = 0xB0
}icu_irq_mode_enum;

#define bsp_irq_mode_set(irq, val)       (R_ICU->IRQCR[irq] = val)

extern void bsp_irq_cfg(void);
